<template>
  <div class="order-right">
    <div class="order-content">
      ...
      <div class="orders">
          <table class="order-item" v-for="(item) in finalData" :key="item.id">
          <thead>
              <tr>
              <th colspan="5">
          ...
          <tbody>
              <tr  v-for="(order,index) in item.orderDetailList" :key="order.id">
                <td width="60%">
                    ...
                    <img :src="order.imgUrl" style="width:80px;height:90px;">
                    <a href="javascript:;" class="block-text">{{order.skuName}}</a>
                    <span>x{{order.skuNum}}</span>
                    <a href="javascript:;" class="service">售后申请</a>
                    </div>
                </td>
                <!-- 统一管理，只需要第一个展示第一次就完事了 -->
...
export default {
  ...
  methods:{
    async getFinal(){
      const result = await this.$API.getFinal()
      if(result.code === 200){
        this.finalData = result.data.records
      }
    },
    reqPost(page){
      console.log('哎，好使',page);
    },
    en(page){
      console.log('en',page);
    }
